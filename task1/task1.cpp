...
public:
    ...
    double get(int index) {
        if (index < 0 || index >= size) return -1.0; // Перевірка виходу за межі
        Node* curr = head; // Ставимо вказівник на початок
        for (int i = 0; i < index; i++) curr = curr->next; // Йдемо до потрібного індексу
        return curr->data; // Повертаємо знайдені дані
    }
    ...
    void insertAt(int index, double value) {
        ...
        if (size == 0) { // Якщо список порожній
            ...
            Node* curr = head; // Починаємо з голови
            for (int i = 0; i < index; i++) curr = curr->next; // Шукаємо вузол, який посунемо
            newNode->next = curr; // Наступним для нового стає знайдений вузол
            newNode->prev = curr->prev; // Попереднім стає той, що був перед знайденим
            curr->prev->next = newNode; // Вузлу зліва кажемо, що наступний тепер новий
            curr->prev = newNode; // Знайденому вузлу кажемо, що попередній тепер новий
        }
    ...
    // ОПЕРАЦІЯ: Видалення за індексом
    void removeAt(int index) {
        if (index < 0 || index >= size) return; // Перевірка меж
        Node* curr = head; // Починаємо з голови
        for (int i = 0; i < index; i++) curr = curr->next; // Шукаємо вузол для видалення

        if (curr->prev) curr->prev->next = curr->next; // Зшиваємо лівого сусіда з правим
        else head = curr->next; // Якщо видаляємо голову, оновлюємо head

        if (curr->next) curr->next->prev = curr->prev; // Зшиваємо правого сусіда з лівим
        else tail = curr->prev; // Якщо видаляємо хвіст, оновлюємо tail

        delete curr; // Звільняємо пам'ять видаленого вузла
